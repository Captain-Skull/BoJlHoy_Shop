...
database.ref('leaveFeedbackText').once('value').then((snapshot) => {
  leaveFeedbackText = snapshot.val() || 'Ваш заказ был выполнен! Спасибо за покупку. Пожалуйста, напишите отзыв в группе и помогите улучшить качество работы. https://t.me/+Q6biAiRMhNszZGUy'
});
...
bot.onText(/\/start(?: (.+))?/, (msg, match) => {
  ...
  if (userBalances[chatId] || userBalances[chatId] === 0) {
    ...
    if (referrerId && referrerId !== chatId.toString() && (userBalances[referrerId] || userBalances[referrerId] === 0)) {
      ...
      // Сохраняем обновлённый объект referralCounts в базе данных
...
Отправьте чек`,
  ...
  if (text === 'Баланс 💳') {
    ...
    const referralLink = `https://t.me/AskySHOP_bot?start=${chatId}`;
